###
#23-10-25
#使用 x 与 y 的值，在将数据分为训练样本集和测试样本集的前提下，通过fisher判别
...
# 线性判别分析
lda_model <- lda(Var_ex_flag ~ x, data = train_data)

# 显示模型结果
lda_model
# 预测
train_pred <- predict(lda_model, newdata = train_data)
...
p1=ggplot(data.frame(fpr_f,tpr_f))+
  ...
  annotate("text", x = 0.25, y = 0.75,
           label = "Test Auc = 0.9697 ", vjust = 1.5, size = 5)+
...
p2=ggplot(data.frame(fpr_f,tpr_f))+
  ...
  ylab("True Positive Rate(TPR)")+
  annotate("text", x = 0.25, y = 0.75,
           label = "Train Auc = 0.9779 ", vjust = 1.5, size = 5)+
  annotate("text", x = 0.75, y = 0.25,
...
ggsave("fisherLDA算法估计分布ROC曲线.tiff",dpi=600, compression = 'lzw')
